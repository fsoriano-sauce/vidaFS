...
def clean_target_url(url: str) -> str:
    ...
    # We use (\?.*)?$ to optionally match query parameters at the end
    patterns = [
        r"/login(\?.*)?$",
        r"/sign-in(\?.*)?$",
        r"/signin(\?.*)?$",
    ...
    # (Docusketch sometimes uses /login or /sign-in, handled above, but being safe)
    if "docusketch" in clean_url.lower():
         clean_url = clean_url.split("/login")[0]
         clean_url = clean_url.split("/sign-in")[0]
